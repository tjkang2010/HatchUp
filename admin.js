...
// Body: { userId, isPremium, days }
// ============================================================
router.post('/premium', auth.adminOnly, (req, res) => {
  try {
    const adapter  = db.getAdapter();
    const adminId  = req.user.userId;
    const { userId, isPremium, days } = req.body;
    ...
    if (isPremium) {
      ...
      premiumUntil = new Date(Date.now() + d * 24 * 60 * 60 * 1000).toISOString();
    }

    adapter.run(
      'UPDATE users SET is_premium=?, premium_until=?, updated_at=? WHERE id=?',
      [isPremium ? 1 : 0, premiumUntil, new Date().toISOString(), userId]
    );

    // 관리자 행동 기록
    db.logAdminAction(adminId, 'PREMIUM',
      `user:${userId}`,
      `프리미엄 ${isPremium ? `부여 (${days||30}일)` : '해제'}`,
      req.ip
    );

    logger.info(`👑 프리미엄 ${isPremium?'부여':'해제'}: 유저 ${userId}, 관리자 ${adminId}`);

    res.json({
      success:      true,
      message:      `프리미엄 ${isPremium ? '부여' : '해제'} 완료!`,
      userId,
      is_premium:   isPremium ? 1 : 0,
      premium_until: premiumUntil,
    });
  } catch (error) {
    logger.error('프리미엄 관리 오류: ' + error.message);
...
// ============================================================
router.get('/premium-users', auth.adminOnly, (req, res) => {
  try {
    const users = db.getAdapter().all(`
      SELECT id, email, nickname, is_premium, premium_until, points, created_at
      FROM users
      WHERE is_premium = 1
