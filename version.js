// ============================================================
// 🎮 버전 시스템 API - version.js
// ============================================================
// 버전 해금, 새 캐릭터 시작, 프레스티지(재시작) 처리
// ============================================================
...
router.get('/status', (req, res) => {
  try {
    ...
    // 각 버전별 해금 여부 + 판매 기록 조회
    const versionStatus = [];

    for (let v = 1; v <= 5; v++) {
      const cfg    = getVersionConfig(v);
      const unlock = checkVersionUnlock(userId, v, database);

      // 이 버전으로 키운 총 캐릭터 수
      const total = adapter.prepare(
        'SELECT COUNT(*) as cnt FROM characters WHERE user_id = ? AND version = ?'
      ).get(userId, v).cnt;

      // 이 버전으로 판매한 수
      const sold = adapter.prepare(`
        SELECT COUNT(*) as cnt
      ...
      // 최고 판매가
      const bestSale = adapter.prepare(`
    ...
    // 현재 활성 캐릭터
    const activeChar = adapter.prepare(`
      SELECT * FROM characters
      WHERE user_id = ? AND is_dead = 0
    ...
    res.json({
      ...
      maxUnlocked: getMaxUnlockedVersion(userId, database),
    });
...
router.post('/start', (req, res) => {
  try {
    ...
    // 해금 조건 확인
    const unlock = checkVersionUnlock(userId, version, database);
    if (!unlock.unlocked) {
    ...
    // 이미 살아있는 캐릭터가 있는지 확인
    const existing = adapter.prepare(
    ...
    const result = adapter.prepare(`
      INSERT INTO characters
        (user_id, name, stage, version, hunger, happy, health, age_days)
      VALUES (?, ?, 'egg', ?, 100, 100, 100, 0)
    `).run(userId, charName, version);
...
router.post('/prestige', (req, res) => {
  try {
    ...
    // 프레스티지 카운트 증가 (users 테이블에 저장)
    adapter.prepare(
      'UPDATE users SET prestige_count = COALESCE(prestige_count, 0) + 1 WHERE id = ?'
    ).run(userId);

    const user         = db.getUserById(userId);
    const prestigeCount = user.prestige_count || 1;

    // 프레스티지 보너스 포인트 지급 (횟수 × 500P)
    const bonusPoints = prestigeCount * 500;
    db.changePoints(userId, bonusPoints, 'earn', `프레스티지 ${prestigeCount}회 달성 보너스`);

    // 1단계 새 캐릭터 자동 생성
    const cfg = getVersionConfig(1);
    adapter.prepare(`
      INSERT INTO characters (user_id, name, stage, version, hunger, happy, health, age_days)
      VALUES (?, ?, 'egg', 1, 100, 100, 100, 0)
    `).run(userId, cfg.stageNames.adult);

    logger.info(`🔄 프레스티지: 유저 ${userId}, ${prestigeCount}회째, +${bonusPoints}P`);

    res.json({
      success: true,
      message: `🔥 프레스티지 ${prestigeCount}회 달성! +${bonusPoints}P 보너스 지급! 1단계부터 다시 시작합니다.`,
      prestigeCount,
      bonusPoints,
    });
...
router.get('/leaderboard', (req, res) => {
  try {
    ...
    // 최고 판매가 기준 랭킹
    const topSellers = adapter.prepare(`
      SELECT
        u.nickname,
    ...
    // 프레스티지 랭킹
    const topPrestige = adapter.prepare(`
...
router.get('/unlock-info/:version', (req, res) => {
  try {
    ...
    const result = checkVersionUnlock(userId, version, database);
    const cfg    = getVersionConfig(version);
